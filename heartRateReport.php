<?php
   ...
   //$res=employeeHealthDetails($_GET['id']);
   $res=empHeartRateDetails($_GET['id']);
   ?>
<html>
   <head>
      <title>Employee Heart Rate Details</title>
      <meta charset="UTF-8">
...
<!----list  the link related to admin action (vertical navbar) ------>
            <div class="col-sm-3 margin">
              <ul class="test">
                <li id="l1" class="active"><a href="heartRateReport.php?id=<?php echo $_GET['id']?>">Heart Rate</a></li>
                <li id="l2"><a href="temperatureReport.php?id=<?php echo $_GET['id']?>"">Temperature</a></li>
              </ul>
             </div>
      ...
      <div class="container">
         <div id="r">
            <div class="row">
               <h1>Employee Heart Rate Details </h1>
               <form class=" navbar-right"><br>
                        ...
                        <?php while($row=mysqli_fetch_array($res,MYSQLI_ASSOC))
                           {
                               ?>
                        <tr>
                           <td><?php echo $row['emp_heart_rate_id']; ?></td>
                           <td><?php echo $row['first_name']; echo ' ';echo $row['last_name']; ?></td>
                           <td><?php echo $row['heart_rate']; ?></td>
                           <td><?php echo $row['heart_rate_status'];?></td>
                           <td><?php echo $row['diesease_prediction'];?></td>
                           <td><?php echo $row['tablet_prediction'];?></td>
                           <td><?php echo $row['heart_rate_mail_status'];?></td>
                           <td><?php echo $row['heart_rate_mail_content'];?></td>
                           <td><?php echo $row['emp_heart_rate_created_date']; ?></td>
                           <td><a href="sendMail.php?heartRateId=<?php echo $row['emp_heart_rate_id'];?>&emailId=<?php echo $row['email_id'];?>"><button type="submit"  id="<?php echo $row['emp_heart_rate_id'];?>"  class="btn btn-info" class="form-control" >Send Mail</button></a></td>
                        </tr>
                        <?php  } ?>
                     </tbody>
                  </table>
